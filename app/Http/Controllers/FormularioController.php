...
class FormularioController extends Controller
{
    ...
    {
        // Intentar obtener columnas de múltiples proyectos existentes para mejor análisis
        $proyectosExistentes = ProyectoProductivo::whereNotNull('data')
            ->where('data->total_rows', '>', 0)
            ->latest()
            ->limit(5) // Revisar los últimos 5 proyectos para mejor análisis
            ->get();

        $columnasAnalizadas = [];

        foreach ($proyectosExistentes as $proyecto) {
            $data = is_string($proyecto->data) ? json_decode($proyecto->data, true) : $proyecto->data;
            $headers = $data['headers'] ?? [];
            $rows = $data['rows'] ?? [];
            ...
            if (!empty($headers) && !empty($rows)) {
                foreach ($headers as $header) {
                    // Excluir columnas automáticas del sistema
                    if ($this->esColumnaAutomatica($header)) {
                        continue;
                    }

                    if (!isset($columnasAnalizadas[$header])) {
                        $columnasAnalizadas[$header] = [
                            'apariciones' => 0,
                            'datos_llenos' => 0,
                            'total_filas' => 0
                        ];
                    ...
                    $columnasAnalizadas[$header]['apariciones']++;
                    $columnasAnalizadas[$header]['total_filas'] += count($rows);

                    // Contar datos no vacíos
                    foreach ($rows as $row) {
                        if (isset($row[$header]) && !empty(trim((string)$row[$header]))) {
                            $columnasAnalizadas[$header]['datos_llenos']++;
                        }
                    }
                }
            }
        ...
        // Filtrar y preparar columnas relevantes
        $columnas = [];
        foreach ($columnasAnalizadas as $nombreColumna => $stats) {
            // Solo incluir columnas que aparecen en al menos 2 proyectos
            // Y que tienen al menos 30% de datos llenos
            $porcentajeLleno = $stats['total_filas'] > 0 ?
                ($stats['datos_llenos'] / $stats['total_filas']) * 100 : 0;

            if ($stats['apariciones'] >= 2 && $porcentajeLleno >= 30) {
                $columnas[] = [
                    'nombre' => $nombreColumna,
                    'tipo' => $this->inferirTipoCampo($nombreColumna),
                    'requerido' => $this->esCampoRequerido($nombreColumna)
                ];
            }
        }

        // Si no hay columnas analizadas, usar columnas por defecto
        if (empty($columnas)) {
            $columnas = [
                ['nombre' => 'CÉDULA', 'tipo' => 'number', 'requerido' => true],
    ...
    /**
     * Verificar si una columna es automática del sistema
     */
    private function esColumnaAutomatica($nombreColumna)
    {
        $columnasAutomaticas = [
        ...
        return in_array($nombreColumna, $columnasAutomaticas);
    }
    ...
    private function inferirTipoCampo($nombreColumna)
    {
        ...
        // CAMPOS NUMÉRICOS - Solo números de documento/ID específicos
        if (str_contains($nombreLower, 'cedula') || str_contains($nombreLower, 'cédula') ||
            str_contains($nombreLower, 'documento identidad') || str_contains($nombreLower, 'dni') ||
            (str_contains($nombreLower, 'id') && !str_contains($nombreLower, 'documentos'))) {
            return 'number';
        }

        // CAMPOS DE TEXTO - Documentos/archivos (no numéricos)
        if (str_contains($nombreLower, 'documentos') || str_contains($nombreLower, 'documento') ||
            str_contains($nombreLower, 'evidencia') || str_contains($nombreLower, 'archivo')) {
            return 'text';
        }
        ...
        if (str_contains($nombreLower, 'genero') || str_contains($nombreLower, 'sexo') || str_contains($nombreLower, 'gender') ||
            ...
            str_contains($nombreLower, 'estado') || str_contains($nombreLower, 'vereda') || str_contains($nombreLower, 'corregimiento')) {
            return 'select';
        }
    ...
    {
        $nombreLower = strtolower($nombreColumna);
        $camposRequeridos = ['cedula', 'cédula', 'nombre', 'name'];

        // Excluir específicamente "documentos" ya que usamos "cédula"
        if (str_contains($nombreLower, 'documentos')) {
            return false;
        }
