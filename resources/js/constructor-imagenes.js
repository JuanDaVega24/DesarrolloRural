/**
 * Funcionalidad de subida de imágenes para el constructor de formularios
 * Permite subir imágenes para las opciones de las preguntas (select y checkbox)
 */

class ConstructorImagenes {
    ...
    init() {
        ...
        // Obtener índice de la pregunta desde el atributo data
        this.preguntaIndex = this.contenedor.dataset.preguntaIndex;
    ...
    procesarArchivos(archivos) {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Guardar imágenes en el formulario
                this.guardarImagenesEnFormulario();
            } else {
    ...
    guardarImagenesEnFormulario() {
        // Crear un campo oculto para almacenar las imágenes de esta pregunta
        const questionItem = this.contenedor.closest('.question-item');
        if (questionItem) {
            const preguntaIndex = questionItem.dataset.questionIndex;
            
            // Buscar o crear el campo oculto para imágenes
            let inputImagenes = questionItem.querySelector(`input[name="preguntas[${preguntaIndex}][imagenes]"]`);
            if (!inputImagenes) {
                inputImagenes = document.createElement('input');
                inputImagenes.type = 'hidden';
                inputImagenes.name = `preguntas[${preguntaIndex}][imagenes]`;
                questionItem.appendChild(inputImagenes);
            }
            
            // Actualizar el valor del campo oculto
            inputImagenes.value = JSON.stringify(this.imagenesActuales);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Inicializar todas las instancias de ConstructorImagenes en la página
    const contenedoresImagenes = document.querySelectorAll('.imagenes-container');
    contenedoresImagenes.forEach(contenedor => {
        new ConstructorImagenes(contenedor);
    });
});

// Exportar para uso en otros módulos
window.ConstructorImagenes = ConstructorImagenes;