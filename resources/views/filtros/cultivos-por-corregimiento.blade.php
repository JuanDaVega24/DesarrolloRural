<x-app-layout>
    ...
    <div class="container-fluid py-4">
        <div class="page-header">
            <div class="header-content">
                <h1>Estadísticas de Cultivos</h1>
                <p>Análisis detallado de los cultivos por corregimiento y vereda.</p>
            </div>
            <div class="header-actions">
                <a href="{{ route('filtros.index') }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left me-1"></i>Volver
                </a>
                <input id="cultivoSearch" class="search-input" type="text" placeholder="Buscar cultivo..." list="cultivoList" />
            </div>
        </div>

        @if(empty($cultivosData))
            <div class="no-data-message">
                <p>No se encontraron datos de cultivos para mostrar.</p>
            </div>
        @else
            <div class="cultivos-grid">
                @foreach($cultivosData as $cultivo => $data)
                    <div class="cultivo-card" data-cultivo="{{ strtolower($cultivo) }}">
                        <div class="card-header">
                            <i class="fas fa-seedling cultivo-icon"></i>
                            <h2 class="cultivo-title">{{ $cultivo }}</h2>
                        </div>
                        <div class="card-body">
                            ...
                            @if(!empty($data['corregimientos']))
                                <button class="expand-btn" onclick="toggleDetails('{{ Str::slug($cultivo) }}')">
                                    <i class="fas fa-chevron-down"></i>
                                    <span>Ver Corregimientos</span>
                                </button>

                                <div id="details-{{ Str::slug($cultivo) }}" class="expand-content">
            ...
            <datalist id="cultivoList">
                @foreach(array_keys($cultivosData) as $c)
    ...
    <script>
        ...
        (function() {
            const input = document.getElementById('cultivoSearch');
            const cards = Array.from(document.querySelectorAll('.cultivo-card'));
            function applyFilter() {
                const term = (input.value || '').toLowerCase().trim();
                cards.forEach(card => {
                    const name = (card.getAttribute('data-cultivo') || '').toLowerCase();
